...
interface IRarityPool is IERC20Upgradeable {
  ...
  /// @notice Query the total available rewards for claim of the supplied user address.
  /// @param _user Address of the user to get rewards.
  /// @return uint256 Amount of $RARE tokens rewarded.
  function getClaimableRewardsForUser(address _user) external view returns (uint256);
  ...
  /// @return uint256 Amount of $RARE tokens rewarded.
  function getClaimableRewardsForRounds(address _user, uint256 _numRounds) external view returns (uint256);
